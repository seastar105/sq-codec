...
def create_fb_matrix(
    n_freqs: int, f_min: float, f_max: float, n_mels: int, sample_rate: int, norm: Optional[str] = None
) -> Tensor:
...
class MelScale(torch.nn.Module):
    r"""Turn a normal STFT into a mel frequency STFT, using a conversion matrix.  This uses triangular filter banks.
    ...
    def __init__(
        ...
        assert f_min <= self.f_max, f"Require f_min: {f_min:f} < f_max: {self.f_max:f}"

        fb = (
            torch.empty(0)
            if n_stft is None
            else create_fb_matrix(n_stft, self.f_min, self.f_max, self.n_mels, self.sample_rate)
        )
...
class TorchSTFT(torch.nn.Module):
    def __init__(
        ...
        self.domain = domain
        self.mel_scale = (
            MelScale(n_mels=(fft_size // 2 + 1), n_stft=(fft_size // 2 + 1), sample_rate=sample_rate)
            if mel_scale
            else None
        )
