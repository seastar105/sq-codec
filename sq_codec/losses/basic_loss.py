# from https://github.com/yangdongchao/UniAudio/blob/main/codec/losses/basic_loss.py
import torch
...
def stft(x, fft_size, hop_size, win_size, window):
    x_stft = torch.view_as_real(torch.stft(x, fft_size, hop_size, win_size, window, return_complex=True))
    real = x_stft[..., 0]
    imag = x_stft[..., 1]
...
class MultiResolutionSTFTLoss(nn.Module):
    def __init__(self, fft_sizes=[1024, 2048, 512], win_sizes=[600, 1200, 240], hop_sizes=[120, 240, 50], **kwargs):
        super().__init__()
