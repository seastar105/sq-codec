# from https://github.com/yangdongchao/UniAudio/blob/main/codec/losses/discriminator_loss.py
import torch
import torch.nn as nn

from sq_codec.losses.basic_loss import MSEDLoss
...
class MSEDiscriminatorLoss(BasicDiscriminatorLoss):
    ...
    def forward(self, real_scores, fake_scores):
        mse_D_loss, mse_D_real_loss, mse_D_fake_loss = self.apply_d_loss(
            scores_fake=fake_scores, scores_real=real_scores, loss_func=self.mse_loss
        )
        return mse_D_loss
